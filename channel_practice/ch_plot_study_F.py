...
def create_1comp_neuron(path,number, h_params, m_params, n_params):
    ...
    """
    Na_h_params, Na_m_params, K_n_params = (h_params, m_params, n_params)
...
def create_model(model, element, h_params, m_params, n_params):
    moose.Neutral('/'+model)
    return create_1comp_neuron('/'+model+'/'+element,1, h_params, m_params, n_params)
...
def current_step_test(simtime, simdt, plotdt, h_params, m_params, n_params):
    ...
    """
    # model creation
    comp = create_model('model', 'neuron', h_params, m_params, n_params)
...
#: The parameters for defining m as a function of Vm
    Na_m_params = [1e5 * (25e-3 + EREST_ACT)+A ,   # 'A_A':
                -1e5,                       # 'A_B':
                -1.0,                       # 'A_C':
...
#are the parameters for the same
    K_n_params = [ 1e4 * (10e-3 + EREST_ACT) + A,   #  'A_A':
               -1e4,                      #  'A_B':
               -1.0,                       #  'A_C':
    ...
    h_params=Na_h_params,
    m_params=Na_m_params, n_params=K_n_params)
